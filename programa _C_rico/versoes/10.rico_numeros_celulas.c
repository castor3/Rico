...
#define TAMANHO_MAX_LINHA 200					//so para se ter uma nocao do tamanho maximo que uma linha pode ter
	...
	for (int i = 0 ; i <= 8; i++){
		...
		while (fgets(line, TAMANHO_TEXTO+1, parameters_file)){			//lê uma linha do ficheiro (ou 500 caracteres, o que chegar primeiro)
			...
			if (strcmp(line, parametros[i]) == 0){	//este é o texto que procuramos, se for igual entra no IF
				...
				fputs(";", newfile);				//passa para a celula a seguir
				while (strcmp(fgets(conversao, 2, parameters_file), fim)){	//le a linha, desta vez na totalidade
					if(inicio == 0)
					...
					if (!*end){						//se conseguiu converter avisa o utilizador e envia para o ficheiro
					...
					if (!strcmp(conversao, ".")){		//pode nao ter encontrado um inteiro, mas os pontos (.) tem que ser alterados para virgulas (,)
						fputs(virgula, newfile);	//e escreve uma virgula no ficheiro, em vez do ponto que encontrou
					}
					inicio = 1;
				}
