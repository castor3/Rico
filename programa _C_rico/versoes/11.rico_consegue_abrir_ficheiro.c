...
#define TAMANHO_MAX_LINHA 40						//so para se ter uma nocao do tamanho maximo que uma linha pode ter
/*
...
// 		p[strlen(p)-1] = 0;
// 	printf(":%s:\n", caminho);
// 	char* str = malloc(sizeof(caminho));
//     strcpy(str, caminho);
//     printf("caminho1:%s\n", caminho);
//     printf("str:%s\n", str);
//     strcpy(caminho, old);
...
//     do{
//     	removeChar(str, 'a');
//     	printf("caminho (no while):%s\n", str);
//     }while(!strcmp(caminho, old));
//     printf("%s\n", caminho);
//     free(str);

// 	if (caminho[sizeof(caminho) - 1] == '\n')		//se o texto que foi lido termina em \n entao
		printf("caracter:%c\n", caminho[sizeof(caminho)-4]);
// 		caminho[sizeof(caminho) - 1] = '\0';		// isso vai ser substituido por \0
	pDir = opendir(caminho);
	printf("caminho:%s:\n", caminho);
    if (pDir == NULL) {
		perror("pDir");
        printf("Erro a aceder ao caminho:\n%s\n", caminho);
        exit(0);
    }
